    ...
    const fetchData = async () => {
      ...
      useEffect(() => {
        ...
        const fetchExercices = async () => {
            if (user && user.id) { // Vérifie si user.id est défini
                try {
                    const response = await axios.get(`http://10.0.2.2:5094/EnvoieExercice/Eleve/${user.id}`);
                ...
                console.log("ID de l'utilisateur non défini, la requête ne sera pas exécutée.");
                setLoading(false);
            }
        };
