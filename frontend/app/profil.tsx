...
const ProfilePage = () => {
      ...
      try {
        ...
        if (value !== null) {
          ...
          const [userResponse, matiereResponse] = await Promise.all([
            axios.get(`http://10.0.2.2:5094/User/Get?id=${userData.id}`),
            axios.get(`http://10.0.2.2:5094/Matiere/${userData.id}`),
          ]);
    ...
    const initialize = async () => {
      try {
        ...
        if (value !== null) {
          ...
          try {
            const response = await  axios.get(`http://10.0.2.2:5094/Level/${userData.id}`);
  ...
  const handleLogout = () => {
    ...
    router.push('/Student/classe');
    // router.push('/login');
  };

  const modifProfil = () => {
    router.push('/modfieProfil');
  };
  ...
  const renderMatiereItem = ({ item }) => (
    <View style={styles.matiereContainer}>
      ...
      <Text style={styles.matiereText}>Matiere: {
    {
      1: "Français",
      2: "Maths",
      3: "Histoire",
  ...
  return (
    <View style={styles.container}>
      <InfoBubble infoText={"votre code : "+user.code} />
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity style={styles.button} onPress={modifProfil}>
          <FontAwesome name="sign-out" size={20} color="#fff" />
